...
export function getPokemon (name) {
    return (dispatch, getState) => API.getInfo(name)
      .then(res => {
      ...
      .catch(error => {
          console.error('erorr with getting pokemon data ', error);
      });
}
...
export function getPokemonsList () {
    return (dispatch, getState) => API.getList()
      .then(res => {
          dispatch(setPokemonsList(res.results));
          return res.results;
      })
      .catch(error => {
          console.error('erorr with getting pokemons list data ', error);
      });
}

export function getPokemonsListBytype (type) {
    return (dispatch, getState) => API.getListBytype(type)
      .then(res => {
          dispatch(setPokemonsList(res.pokemon));
          return res;
      })
      .catch(error => {
          console.error('erorr with getting pokemons list by type data ', error);
      });
}
